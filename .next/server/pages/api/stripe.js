...
const stripe = new (external_stripe_default())(process.env.SECRET_STRIPE_KEY);
  ...
  if (req.method === "POST") {
    try {
      const params = {
        ...
        shipping_options: [{
          shipping_rate: "shr_1MTqFAFCMgEQwloMtQqIcJ4H"
        }],
        line_items: req.body.map(item => {
          const img = item.image[0].asset._ref;
          const newImage = img.replace("image-", "https://cdn.sanity.io/images/hgxyuyds/production/").replace("-webp", ".webp");
          return {
            price_data: {
              currency: "usd",
              product_data: {
